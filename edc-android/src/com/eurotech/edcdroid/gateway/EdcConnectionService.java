...
import com.eurotech.edcdroid.tools.SensorData;
...
public class EdcConnectionService extends Service implements EdcCallbackHandler {
	...
	public static final String messageTitle = "EDC";
	...
	public void onCreate() {
		...
		if (!networkIsConnected(localContext)) {
			...
			EdcNotification(localContext, logo, messageTitle, message, Notification.FLAG_NO_CLEAR, Demo.class);
			stopSelf();
		}
	...
	void handleStart(Intent intent, int startId) {
		...
		try {
			...
			String message = "Unable to instantiate EDC client. Check EDC settings.";
			EdcNotification(localContext, logo, messageTitle, message, Notification.FLAG_NO_CLEAR, Demo.class);
			stopSelf();
			return;
		}
	...
	private boolean edcConnect() {
		...
		try {
			...
			Demo.localLog("END START", Demo.ENABLE);
			return true;
		} catch (EdcClientException e) {
			Demo.localLog("END EXCEPTION", Demo.ENABLE);
			return false;
		}
	}
	...
	public void EdcDisconnect() {
		...
		try {
			if (edcClient != null) {
				...
				// give enough time to publish the death certificate
				Thread.sleep(100);
				// then disconnect
				edcClient.terminate();
			}
	...
	public static String networkType() {
		Demo.localLog("BEGIN", Demo.ENABLE);
		ConnectivityManager cm = (ConnectivityManager) Demo.startingActivityContext.getSystemService(Context.CONNECTIVITY_SERVICE);
		Demo.localLog("END", Demo.ENABLE);
			...
			if (currentNetworkInfo != null) {
				...
				case CONNECTED:
					if (firstConnection) {
						...
						EdcNotification(context, logo, messageTitle, message, Notification.FLAG_NO_CLEAR, Demo.class);
					}
					break;
				case DISCONNECTED:
					int logo = R.drawable.redlogo;
	...
	public static void EdcNotification(Context context, int logo, String title, String message, int notificationFlags, Class<?> notificationClass) {
		Demo.localLog("BEGIN", Demo.ENABLE);
		NotificationManager nm = (NotificationManager) context.getSystemService(NOTIFICATION_SERVICE);
		Notification notification = new Notification(logo, message, System.currentTimeMillis());
	...
	private class EdcMessageReceiverManager extends BroadcastReceiver {
		...
		public EdcMessageReceiverManager() {
			...
			for (String topic : Demo.semanticTopicList) {
