...
import com.eurotech.edcandroid.commands.Command;
import com.eurotech.edcandroid.gateway.EdcConnectionIntents;
import com.eurotech.edcandroid.gateway.EdcConnectionService;
import com.eurotech.edcandroid.gateway.EdcReceiveActivity;
import com.eurotech.edcandroid.gateway.EdcSubscribeActivity;
import com.eurotech.edcandroid.gateway.LocalSendBroadcast;
import com.eurotech.edcandroid.settings.SettingsActivity;
...
public class EDCAndroid extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
				...
				case TOPICS:
					if (!selectedTile.get(sel).active) break;
					LinearLayout layout = new LinearLayout(EDCAndroid.this);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_about:
			...
			String release = null;
			try {
				release = getPackageManager().getPackageInfo(getPackageName(), 0).versionName;
			} catch (NameNotFoundException e) {
				e.printStackTrace();
			}
			builder.setMessage("Release: " + release + "\n" + "Build Date: " + builddate);
			...
			public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
				...
				if (keyList.contains(key)) {
					...
					if (key.equals("Cloud connection")) {
						EdcConnectionService.deviceData = new DeviceData(EDCAndroid.this);
						if (EdcConnectionService.deviceData.connectivity && allSetsDone) {
							startService(new Intent(EDCAndroid.this, EdcConnectionService.class));				
						}
						if (!EdcConnectionService.deviceData.connectivity) {
							stopService(new Intent(EDCAndroid.this, EdcConnectionService.class));
							int logo;
							String message;
							if (EdcConnectionService.networkIsConnected(startingActivityContext)) {
								logo = R.drawable.yellowlogo;
	...
	public static void localLog(String message, String assetID) {
		...
		String methodName = e.getMethodName();
		String className = e.getClassName();
		Log.d(assetID, className.substring(className.lastIndexOf(".") + 1) + " " + methodName + ": " + message);
		if (logFile != null) {
			try {
				String messageToSend = assetID + ": " + className.substring(className.lastIndexOf(".") + 1) + " " + methodName + ": " + message + "\n";
				messageToSend.replaceAll(controlString, "");
				logFileWriter.writeChars(messageToSend);
			} catch (IOException e1) {
				int logo = R.drawable.redlogo;
